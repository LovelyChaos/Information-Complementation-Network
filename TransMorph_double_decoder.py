...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, patch_size=4, in_chans=3, embed_dim=96, norm_layer=None):
        ...
        #self.proj = nn.Conv3d(in_chans, embed_dim, kernel_size = 7, stride = 4,padding = (3, 3, 3))
        self.proj = nn.Conv3d(in_chans, embed_dim, kernel_size = 5, stride = 4, padding = (2, 2, 2))
...
class DecoderBlock_main(nn.Module):
    def __init__(
            self,
            in_channels,
            out_channels,
        ...
        super().__init__()
        self.conv1 = Conv3dReLU(
            in_channels + skip_channels + vice_channels,
            out_channels,
            kernel_size=3,
            padding=1,
            use_batchnorm=use_batchnorm,
    ...
    def forward(self, x, vice, skip=None):
        x = self.up(x)
        if skip is not None:
            x = torch.cat([x, vice], dim=1)
            x = torch.cat([x, skip], dim = 1)
        x = self.conv1(x)
        x = self.conv2(x)
        return x
...
class TransMorph(nn.Module):
    def __init__(self, config):
        ...
        self.main1 = DecoderBlock_main(embed_dim * 4, embed_dim * 2, vice_channels = embed_dim*2,
                                       skip_channels = embed_dim * 2 if if_transskip else 0, use_batchnorm = False)
        self.main2 = DecoderBlock_main(embed_dim * 2, embed_dim , vice_channels = embed_dim,
                                       skip_channels = embed_dim if if_transskip else 0, use_batchnorm = False)
        self.main3 = DecoderBlock_main(embed_dim , embed_dim // 2, vice_channels = embed_dim // 2,
                                       skip_channels = embed_dim // 2 if if_transskip else 0, use_batchnorm = False)
        self.main4 = DecoderBlock_main(embed_dim // 2, config.reg_head_chan, vice_channels = config.reg_head_chan,
                                       skip_channels = config.reg_head_chan if if_transskip else 0, use_batchnorm = False)
    ...
    def forward(self, x):
        ...
        '''vice只有4个，从下到上是0-3'''
        vice0 = self.up1(f1, f2)#
        vice1 = self.up2(vice0, f3)  #
        vice2 = self.up3(vice1, f4)  #
        vice3 = self.up4(vice2, f5)  #

        '''main有5个，从下到上是0-4'''
        x = self.up0(out_feats[-1], f1)#[2, 384, 10, 12, 14]
        x = self.main1(x, vice0, f2)#[2, 192, 20, 24, 28]
        x = self.main2(x, vice1, f3)#[2, 96, 40, 48, 56]
        x = self.main3(x, vice2, f4)#[2, 48, 80, 96, 112]
        x = self.main4(x, vice3, f5)#[2, 16, 160, 192, 224]
